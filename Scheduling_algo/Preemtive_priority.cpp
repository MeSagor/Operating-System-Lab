...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        cin >> readyQueue[i].burst_time >> readyQueue[i].arrival_time >> readyQueue[i].priority_value;
    ...
    while (completed != n) {
        int selectedProcessIdx = -1, min_priority_value = 1000;
        for (int i = 0; i < n; i++) {
            if (!readyQueue[i].is_completed and readyQueue[i].arrival_time <= time) {
                if (readyQueue[i].priority_value < min_priority_value) {
                    min_priority_value = readyQueue[i].priority_value;
                    selectedProcessIdx = i;
                } else if (readyQueue[i].priority_value == min_priority_value) {
